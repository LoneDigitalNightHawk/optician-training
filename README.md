...
Interactive training for Visual Health Doctors of Optometry, built as plain self-contained HTML with no build step and no dependencies. Push to `master` and GitHub Pages redeploys automatically in ~1 minute.
...
1. Copy `module1.html` to `moduleN.html`; it's the cleanest template (styles and quiz engine in one file).
...
Look for the gold **"Training team: to complete"** callout boxes inside Module 1. Those mark the real content gaps (org chart, office contacts, system provisioning) still waiting on the team.
